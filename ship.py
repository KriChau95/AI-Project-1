    ...
    while fire_q:
        ...
        for x in range(len(fire_q)):
                ...
                if result=="fire":
                    ...
                    inQueue.add((row,col))
        temp = np.array(fire)  # Shape (40, 40)
        temp = temp.reshape(1, 40, 40)  # Sha   pe (1, 40, 40)
        fire_prog = np.concatenate((fire_prog, temp), axis=0)  # Now shape (2, 40, 40)
        
    # for ship in fire_prog:
